...
def get_kg_retriever() -> LegalRetriever:
    """
    ...
    if _retriever is None:
        with _RETRIEVER_LOCK:
            if _retriever is None:
                ...
                logger.info("LegalRetriever singleton created successfully.")
    return _retriever
...
# ── Status ────────────────────────────────────────────────────────────────────
@kg_retriever_router.get("/status", response_model=RetrieverStatusResponse, summary="Check whether the LegalRetriever singleton is initialised")
def retriever_status() -> RetrieverStatusResponse:
    if _retriever is None:
        return RetrieverStatusResponse(retriever_ready=False)
    
    # Safe inspection of the private BM25 structures if initialized
    bm25_count = None
    if hasattr(_retriever, "_bm25_index") and _retriever._bm25_index is not None:
        if hasattr(_retriever._bm25_index, "_records"):
...
# ── Retrieve ─────────────────────────────────────────────────────────────────
@kg_retriever_router.post("/retrieve", response_model=RetrieveResponse, summary="Hybrid legal retrieval: vector ANN + BM25 RRF + graph expansion")
def kg_retrieve(req: RetrieveRequest, retriever: LegalRetriever = Depends(get_kg_retriever)) -> RetrieveResponse:
    try:
...
# ── Index management ──────────────────────────────────────────────────────────
@kg_retriever_router.post("/index/setup", response_model=IndexSetupResponse, summary="Create Neo4j vector indexes for Article and TableChunk nodes")
def setup_index(
    dimensions: Optional[int] = Query(None, description="Override embedding dimensions (auto-inferred if omitted)"),
    retriever: LegalRetriever = Depends(get_kg_retriever)
) -> IndexSetupResponse:
    try:
...
@kg_retriever_router.post("/index/embed", response_model=EmbedResponse, summary="Embed all un-embedded Article and TableChunk nodes")
def embed_nodes(
    ...
    try:
        ...
        return EmbedResponse(
            message       = "Embedding complete." if n else "No new nodes to embed.",
            nodes_written = n,
        )
    except Exception as exc:
...
@kg_retriever_router.post("/index/reindex", response_model=EmbedResponse, summary="Force re-embed all Article nodes (overwrites existing embeddings)")
...
@kg_retriever_router.post("/index/rebuild", response_model=EmbedResponse, summary="Drop, recreate, and fully re-embed both Neo4j vector indexes")
def rebuild_index(
    batch_size:  int            = Query(128, ge=1),
    max_workers: int            = Query(4,   ge=1),
